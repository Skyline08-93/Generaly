...
# Улучшенная инициализация Telegram
telegram_app = None
...
try:
    ...
    if TELEGRAM_TOKEN and TELEGRAM_CHAT_ID:
        # Попытка создания тестового бота для проверки токена
        test_bot = Bot(token=TELEGRAM_TOKEN)
        await test_bot.get_me()  # Проверка токена
        
        # Если проверка прошла успешно, создаем основное приложение
        telegram_app = Application.builder().token(TELEGRAM_TOKEN).build()
        telegram_ready = True
        print("✅ Telegram бот успешно инициализирован")
    else:
        telegram_error = "Не хватает TELEGRAM_TOKEN или TELEGRAM_CHAT_ID"
except Exception as e:
    telegram_error = f"Ошибка инициализации Telegram: {str(e)}"
    telegram_ready = False

if not telegram_ready:
    print(f"⚠️ {telegram_error}")
    print("Уведомления в Telegram будут отключены")
...
async def main():
    try:
        ...
        print(f"Telegram: {'подключен' if telegram_ready else 'отключен'}")
        if not telegram_ready and telegram_error:
            print(f"Причина: {telegram_error}")
        print("="*50)
